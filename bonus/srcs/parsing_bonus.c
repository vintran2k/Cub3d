...
/*   Updated: 2021/03/25 01:40:53 by vintran          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if ((var->f != -1 && var->ft != NULL) || (var->c != -1 && var->ct != NULL))
		ft_error(var, "Invalid precision in .cub\n");
	if (var->no == NULL || var->so == NULL || var->we == NULL ||
	var->ea == NULL || var->sp == NULL || !var->rx || !var->ry)
		return (0);
	if ((var->f == -1 && var->ft == NULL) || (var->c == -1 && var->ct == NULL))
		return (0);
	return (1);
...
void	get_map_params(char *line, t_var *var)
{
	...
	if (line[0] == 'S' && line[1] == 'O')
		...
		get_path_texture(var, line, &var->ft);
	else if (line[0] == 'C' && line[1] == 'T')
		get_path_texture(var, line, &var->ct);
