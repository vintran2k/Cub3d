...
/*   Updated: 2021/03/08 16:12:07 by vintran          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../inc/cub3d.h"

void	init_delta(double *deltax, double *deltay,
						double raycstdirx, double raycstdiry)
{
	if (raycstdiry == 0)
		*deltax = 0;
		...
		*deltax =
			sqrt(1 + (raycstdiry * raycstdiry) / (raycstdirx * raycstdirx));
	if (raycstdirx == 0)
		*deltay = 0;
		...
		*deltay =
			sqrt(1 + (raycstdirx * raycstdirx) / (raycstdiry * raycstdiry));
}
...
void	init_step_sidedist(t_var *var)
{
	...
	{
		var->raycst.stepx = -1;
		var->raycst.sidedistx =
			(var->raycst.posx - var->raycst.mapx) * var->raycst.deltadistx;
	}
	else
	{
		var->raycst.stepx = 1;
		var->raycst.sidedistx =
		(var->raycst.mapx + 1.0 - var->raycst.posx) * var->raycst.deltadistx;
	}
	if (var->raycst.raydiry < 0)
	{
		var->raycst.stepy = -1;
		var->raycst.sidedisty =
			(var->raycst.posy - var->raycst.mapy) * var->raycst.deltadisty;
	}
	else
	{
		var->raycst.stepy = 1;
		var->raycst.sidedisty =
		(var->raycst.mapy + 1.0 - var->raycst.posy) * var->raycst.deltadisty;
	}
}
...
void	init_raycasting(t_var *var, int x)
{
	...
	var->raycst.raydirx =
		var->raycst.dirx + var->raycst.planex * var->raycst.camerax;
	var->raycst.raydiry =
		var->raycst.diry + var->raycst.planey * var->raycst.camerax;
	var->raycst.mapx = (int)var->raycst.posx;
	var->raycst.mapy = (int)var->raycst.posy;
	var->raycst.hit = 0;
	init_delta(&var->raycst.deltadistx, &var->raycst.deltadisty,
			var->raycst.raydirx, var->raycst.raydiry);
